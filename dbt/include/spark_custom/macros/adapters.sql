...
{% macro spark_custom__create_table_as(temporary, relation, sql) -%}
  {% if temporary -%}
...
{% macro spark_custom__create_view_as(relation, sql) -%}
...
{% macro spark_custom__create_schema(relation) -%}
  {%- call statement('create_schema') -%}
    create schema if not exists {{relation}}
  {% endcall %}
{% endmacro %}

{% macro spark_custom__drop_schema(relation) -%}
  {%- call statement('drop_schema') -%}
    drop schema if exists {{ relation }} cascade
  {%- endcall -%}
{% endmacro %}

{% macro spark_custom__get_columns_in_relation(relation) -%}
...
{% macro spark_custom__list_relations_without_caching(relation) %}
...
{% macro spark_custom__list_schemas(database) -%}
...
{% macro spark_custom__current_timestamp() -%}
  current_timestamp()
{%- endmacro %}

{% macro spark_custom__rename_relation(from_relation, to_relation) -%}
...
{% macro spark_custom__drop_relation(relation) -%}
...
{% macro spark_custom__generate_database_name(custom_database_name=none, node=none) -%}
  {% do return(None) %}
{%- endmacro %}

{% macro spark_custom__persist_docs(relation, model, for_relation, for_columns) -%}
  {% if for_columns and config.persist_column_docs() and model.columns %}
    {% do alter_column_comment(relation, model.columns) %}
  {% endif %}
{% endmacro %}

{% macro spark_custom__alter_column_comment(relation, column_dict) %}
...
{% macro spark_custom__make_temp_relation(base_relation, suffix) %}
...
{% macro spark_custom__alter_column_type(relation, column_name, new_column_type) -%}
...
{% macro spark_custom__alter_relation_add_remove_columns(relation, add_columns, remove_columns) %}
